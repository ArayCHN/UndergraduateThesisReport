...
\section{理论基础}
  ...
  \subsection{有模型和无模型结合的高效强化学习}
    ...
    2017年，Bansal等人提出MBMF算法（Model-Based Priors for Model-Free RL）\cite{MBMF}，首先训练出系统的动力学特性，作为一个函数$s_{t+1} = f(s_t, a)$，把这个模型方程$f$作为后续无模型强化学习训练的预先输入，相当于初始值比较准确。\par
  ...
  MPC的方法如下：
  \par 在RL的研究中，有不少利用/结合MPC的算法（在这里找一些！！我的论文中本来就有一些，pieter abbeel也有一些无人机的论文！！）。
